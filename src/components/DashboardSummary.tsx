  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <SummaryTile label="RSVPs">
        {total === 0 ? (
          <p className="text-sm text-foreground/75">No guests added yet</p>
        ) : (
          <>
        ...
        {total === 0 ? (
          ...
          <>
            <p className="text-2xl font-semibold text-brand">
              {checkedIn}
              <span className="text-base text-foreground/70 font-normal"> / {total}</span>
            </p>
      ...
      <SummaryTile label="Shuttles">
        {shuttles.length === 0 ? (
          <p className="text-sm text-foreground/75">No shuttles yet</p>
        ) : (
          <>
            <p className="text-2xl font-semibold text-brand">
              {liveShuttles}
              <span className="text-base text-foreground/70 font-normal"> live</span>
            </p>
            <p className="text-xs text-foreground/75 mt-1">{shuttles.length} total</p>
          </>
        )}
      </SummaryTile>
    </div>
...
function SummaryTile({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="bg-white border border-border-warm rounded-xl p-5">
      <p className="text-xs font-medium text-foreground/75 uppercase tracking-wide mb-2">{label}</p>
      {children}
    </div>
  );
}
